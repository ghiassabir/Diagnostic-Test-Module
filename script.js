...
function startConfetti() {
    if (!confettiCanvas || !finishedViewEl || !finishedViewEl.classList.contains('active')) {
        return; // Don't run if canvas not found or view not active
    }
    ...
    class Particle {
        constructor(x, y) {
            ...
            this.directionX = (Math.random() * 2 - 1) * 2;
            this.color = colors[Math.floor(Math.random() * colors.length)];
        }
        update() {
            this.y += this.weight;
            this.x += this.directionX;
            if (this.y > confettiCanvas.height) { this.y = 0 - this.size; this.x = Math.random() * confettiCanvas.width; }
            if (this.x > confettiCanvas.width) { this.x = 0 - this.size; }
            if (this.x < 0 - this.size) { this.x = confettiCanvas.width; }
        }
        draw() {
            ctx.fillStyle = this.color;
            ctx.beginPath();
            ctx.rect(this.x, this.y, this.size, this.size * 1.5);
            ctx.closePath();
            ctx.fill();
        }
    }
    ...
    function initConfetti() {
        ...
        for (let i = 0; i < 150; i++) {
            confettiParticles.push(new Particle(Math.random() * confettiCanvas.width, Math.random() * confettiCanvas.height - confettiCanvas.height));
        }
    }

    function animateConfetti() {
        if (!finishedViewEl.classList.contains('active')) { // Stop animation if we leave the finished view
...
function loadQuestion() {
    ...
    // Set headers and tool visibility
    if(sectionTitleHeader) sectionTitleHeader.textContent = `Section ${currentModuleIndex + 1}: ${currentModuleInfo.name}`;
    if(questionNumberBoxMainEl) questionNumberBoxMainEl.textContent = currentQuestionDetails.question_number || currentQuestionNumber;
    ...
    if (questionImage) {
        ...
        console.warn("DEBUG loadQuestion: questionImage DOM element not found.");
    }

    const passageTextFromJson = currentQuestionDetails.passage_content;
    const stemTextFromJson = currentQuestionDetails.question_stem;

    // --- LOGIC FOR PANE LAYOUT AND CONTENT ---
    if (currentQuestionDetails.question_type === 'student_produced_response') {
        mainContentAreaDynamic.classList.remove('single-pane');
        sprInstructionsPane.style.display = 'flex';
        paneDivider.style.display = 'block';
        ...
        for (const [key, value] of Object.entries(options)) {
            const isSelected = (answerState.selected === value);
            const isCrossedOut = answerState.crossedOut.includes(key);
            // ... (The rest of your existing logic to create and append option elements is correct and can stay here) ...
            const containerDiv = document.createElement('div');
            containerDiv.className = 'answer-option-container';
            containerDiv.dataset.optionKey = key;
            const optionDiv = document.createElement('div');
